...
* `.antigravitycli/hooks.json` runs `.hooks/prek-lint.sh --adapter antigravity` during `PostInvocation`. The shared hook runner uses `.pre-commit-config.yaml` as the canonical Prek validation config and translates failures into hook block decisions.
