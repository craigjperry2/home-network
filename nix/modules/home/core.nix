  ...
  programs = {
    ...
    zsh = {
      ...
      shellAliases = {
        ...
        why-awake = ''ssh s1.home.craigjperry.com "last_wake=\$(sudo journalctl -u systemd-suspend.service --since '1 week ago' --no-pager | awk '/System returned from sleep/ {last=\$1\\\" \\\"\$2\\\" \\\"\$3} END {print last}'); if [ -n \\\"\$last_wake\\\" ]; then wake_ts=\$(date -d \\\"\$last_wake\\\" +%s); now_ts=\$(date +%s); diff=\$((now_ts - wake_ts)); hours=\$((diff / 3600)); mins=\$(( (diff % 3600) / 60 )); [ \$hours -gt 0 ] && printf 'Awake for %dh %dm\\n' \$hours \$mins || printf 'Awake for %dm\\n' \$mins; else up=\$(cat /proc/uptime | awk '{print int(\$1)}'); days=\$((up / 86400)); hours=\$(( (up % 86400) / 3600 )); mins=\$(( (up % 3600) / 60 )); [ \$days -gt 0 ] && printf 'Awake for %dd %dh %dm\\n' \$days \$hours \$mins || ([ \$hours -gt 0 ] && printf 'Awake for %dh %dm\\n' \$hours \$mins || printf 'Awake for %dm\\n' \$mins); fi; sudo journalctl -u autosuspend.service -n 1 --no-pager | awk '{print \\\"Last check at: \\\" \$1 \\\" \\\" \$2 \\\" \\\" \$3}'; echo 'Active checks (last 50 mins):'; sudo journalctl -u autosuspend.service -n 50 --no-pager | awk '/Check .* matched/ { ts=\$1\\\" \\\"\$2\\\" \\\"\$3; check=gensub(/.*Check (.*) matched.*/, \\\"\\\\\\\\1\\\", \\\"g\\\", \$0); if (!(check in start)) start[check]=ts; } END { for (c in start) { print \\\"  - \\\" c \\\" (active since \\\" start[c] \\\")\\\"; } }'" '';
